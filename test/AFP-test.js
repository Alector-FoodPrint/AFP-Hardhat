...
describe('Alector FoodPrint', () => {
  ...
  beforeEach(async function () {
    ...
    await afp.connect(producer).Produce(InputAsset.name, InputAsset.quantity);
    await afp.connect(producer).Produce(InputAsset2.name, InputAsset2.quantity);
  });
  ...
  describe('Transfer FoodAsset', async function () {
    it('Producer has a registered food assset ', async function () {
      ...
      const assetName = asset.name; // tomatoes
      const assetQuantity = asset.quantity; // ??

      const assetQuantityBN = ethers.BigNumber.from(assetQuantity); // 100 in BigNum ??
      const assetQuantityStr = assetQuantityBN.toString(); // '100'

      console.log(assetQuantityStr);

      expect(await assetName).to.equal(InputAsset.name);
      expect(await assetQuantityStr).to.equal(`${InputAsset.quantity}`);
    });
  ...
  describe('List NFTs of owner', async function () {
    it('List NFTs by produced by producer', async function () {
      ...
      // console.log(NFTList);
      expect(await NFTList[0].name).to.equal('tomatoes');
      expect(await NFTList[1].name).to.equal('potatoes');
